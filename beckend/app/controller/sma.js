...
module.exports = {
    ...
    getSdById : (req, res) => {
        const qstring =`SELECT * FROM sma WHERE id = '${req.params.id}'`
        connection.query(qstring, (err, data)=>{
            if (err) {
                console.log("error", err)
                res.status(500).send({
    ...
    postSd : (req, res) => {
        ...
        connection.query("INSERT INTO sma SET ?", data, (err)=>{
            if (err){
    ...
    updateSd : (req, res) => {
        ...
        const data = req.body
        const qstring = `UPDATE sma SET ? WHERE id = ${id}`;
    ...
    deleteSd : (req, res) => {
        const id = req.params.id;
        const qstring = `DELETE FROM sma WHERE id = ${id}`
        connection.query(qstring, (err, data)=>{
            if (err) {
                res.status(500).send({
